    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 space-y-6">
        ...
        <div class="bg-gradient-to-r from-sky-800 via-sky-900 to-slate-900 rounded-xl p-6 md:p-8 text-white shadow-lg flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
            <div>
                ...
                <h1 class="text-2xl md:text-3xl font-bold font-title mt-2">👔 Kelola Perangkat Desa</h1>
                <p class="text-slate-300 text-sm mt-1">Mengatur jajaran perangkat desa, foto profil, jabatan, dan urutan penampilan di halaman profil desa.</p>
            </div>
            <a href="{{ route('tentang') }}" class="px-5 py-2.5 bg-white/10 hover:bg-white/20 text-white font-semibold rounded-xl backdrop-blur-sm border border-white/20 transition flex items-center gap-2 text-sm shrink-0">
                <i class="fa-solid fa-arrow-left"></i> Kembali ke Halaman Profil
            </a>
        </div>
        ...
        <div class="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-4 bg-white p-6 rounded-2xl shadow-sm border border-slate-200">
            <div>
                ...
                <h2 class="text-lg font-bold text-slate-800 font-title mt-0.5">Daftar Perangkat Desa</h2>
            </div>
            <div>
                <a href="{{ route('admin.perangkat-desa.create') }}" class="inline-flex items-center justify-center px-4 py-2 bg-sky-600 hover:bg-sky-700 text-white text-sm font-semibold rounded-xl transition shadow">
        ...
        <div class="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden">
            @if($perangkat->isEmpty())
                <div class="text-center text-slate-500 py-16 px-4">
                    ...
                    <p class="text-sm font-medium">Belum ada data perangkat desa. Silakan tambahkan perangkat desa pertama.</p>
                </div>
            @else
                    ...
                    <table class="w-full text-sm">
                        <thead class="bg-slate-50 border-b border-slate-200 text-left">
                            <tr>
                                ...
                                <th class="px-6 py-4 font-semibold text-slate-600 uppercase text-xs tracking-wider">Nama Perangkat</th>
                                <th class="px-6 py-4 font-semibold text-slate-600 uppercase text-xs tracking-wider">Jabatan</th>
                        ...
                        <tbody class="divide-y divide-slate-100">
                            @foreach($perangkat as $p)
                                <tr class="hover:bg-slate-50/80 transition">
                                    <td class="px-6 py-4">
                                        @if($p->foto)
                                            <img src="{{ (str_starts_with($p->foto, 'http') || str_contains($p->foto, 'storage/')) ? asset($p->foto) : Storage::url($p->foto) }}" 
                                                 class="w-12 h-12 object-cover rounded-xl border border-slate-200 shadow-sm" 
                                                 alt="Foto {{ $p->nama }}">
                                        @else
                                            <div class="w-12 h-12 bg-sky-50 text-sky-700 rounded-xl flex items-center justify-center text-sm font-bold border border-slate-200">
                                    ...
                                    <td class="px-6 py-4">
                                        <div class="flex justify-end gap-2">
                                            <a href="{{ route('admin.perangkat-desa.edit', $p->id) }}"
                                            ...
                                            <form action="{{ route('admin.perangkat-desa.destroy', $p->id) }}" method="POST"
                                                  onsubmit="return confirm('Hapus data perangkat desa {{ $p->nama }}?')">
                                                @csrf
                                                @method('DELETE')
                                                <button type="submit"
