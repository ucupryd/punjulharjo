    ...
    <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 space-y-6">
        ...
        <div class="bg-gradient-to-r from-sky-800 via-sky-900 to-slate-900 rounded-xl p-6 md:p-8 text-white shadow-lg flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
            <div>
                ...
                <h1 class="text-2xl md:text-3xl font-bold font-title mt-2">👔 Tambah Perangkat Desa</h1>
                <p class="text-slate-300 text-sm mt-1">Tambahkan anggota perangkat pemerintahan desa yang baru.</p>
            </div>
        ...
        <!-- Form Card -->
        <form action="{{ route('admin.perangkat-desa.store') }}" method="POST" enctype="multipart/form-data" class="bg-white p-8 rounded-2xl shadow-sm border border-slate-200 space-y-6">
            @csrf
            ...
            <div>
                <label class="block text-xs font-semibold text-slate-600 uppercase mb-1.5">Foto Profil Perangkat (Opsional)</label>
                ...
                <p class="text-[11px] text-slate-400 mt-1.5">Format: JPG, JPEG, PNG, WEBP. Ukuran file maksimal: 5MB.</p>
            </div>
