    ...
    <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 space-y-6">
        ...
        <div class="bg-gradient-to-r from-sky-800 via-sky-900 to-slate-900 rounded-xl p-6 md:p-8 text-white shadow-lg flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
            <div>
                ...
                <h1 class="text-2xl md:text-3xl font-bold font-title mt-2">🎟️ Edit Tarif Tiket</h1>
                <p class="text-slate-300 text-sm mt-1">Ubah data tarif tiket atau biaya wahana Pantai Karang Jahe.</p>
            </div>
            <a href="{{ route('admin.moderasi.index', ['tab' => 'tiket']) }}" class="px-5 py-2.5 bg-white/10 hover:bg-white/20 text-white font-semibold rounded-xl backdrop-blur-sm border border-white/20 transition flex items-center gap-2 text-sm shrink-0">
        ...
        <!-- Form Card -->
        <form action="{{ route('admin.karang-jahe-tiket.update', $tiket->id) }}" method="POST" class="bg-white p-8 rounded-2xl shadow-sm border border-slate-200 space-y-6">
            @csrf
            @method('PUT')
            
            <div>
                <label class="block text-xs font-semibold text-slate-600 uppercase mb-1.5">Komponen Tiket / Wahana</label>
                <input type="text" name="komponen" required value="{{ old('komponen', $tiket->komponen) }}" 
            ...
            <div>
                ...
                <input type="text" name="tarif" required value="{{ old('tarif', $tiket->tarif) }}" 
                       class="w-full border border-slate-300 rounded-xl px-4 py-2.5 text-sm focus:ring-2 focus:ring-sky-500 focus:outline-none placeholder-slate-400" 
            ...
            <div>
                ...
                <input type="text" name="catatan" value="{{ old('catatan', $tiket->catatan) }}" 
                       class="w-full border border-slate-300 rounded-xl px-4 py-2.5 text-sm focus:ring-2 focus:ring-sky-500 focus:outline-none placeholder-slate-400" 
            ...
            <div>
                ...
                <input type="text" name="ikon" value="{{ old('ikon', $tiket->ikon) }}" 
            ...
            <div>
                ...
                <input type="number" name="urutan" value="{{ old('urutan', $tiket->urutan) }}" min="1"
                       class="w-full border border-slate-300 rounded-xl px-4 py-2.5 text-sm focus:ring-2 focus:ring-sky-500 focus:outline-none placeholder-slate-400">
            </div>

            <div class="pt-4 border-t border-slate-100 flex justify-end gap-3">
                <a href="{{ route('admin.moderasi.index', ['tab' => 'tiket']) }}" class="px-5 py-2.5 bg-slate-100 text-slate-700 font-semibold text-sm rounded-xl hover:bg-slate-200 transition">
                    Batal
                </a>
