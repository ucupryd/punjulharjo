    ...
    <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 space-y-6">
        ...
        <form action="{{ route('admin.blog.store') }}" method="POST" enctype="multipart/form-data" class="bg-white p-8 rounded-2xl shadow-sm border border-slate-200 space-y-6">
            ...
            <div>
                ...
                <textarea name="excerpt" rows="2" class="w-full border border-slate-300 rounded-xl px-4 py-2.5 text-sm focus:ring-2 focus:ring-sky-500 focus:outline-none" placeholder="Ringkasan singkat artikel (opsional)...">{{ old('excerpt') }}</textarea>
            </div>
            ...
            <div>
                ...
                <textarea name="content" required rows="10" class="w-full border border-slate-300 rounded-xl px-4 py-2.5 text-sm focus:ring-2 focus:ring-sky-500 focus:outline-none" placeholder="Tuliskan lengkap isi konten artikel di sini...">{{ old('content') }}</textarea>
            </div>
